# src/inference/services/goals_away_service.py
...
@dataclass
class GoalsAwayResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet gólů REQUEST-AWAY (lambda)
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class GoalsAwayService:
    """
    Service pro goals_away = góly hostů dle REQUESTU.

    Pozn.: pokud _pick_row vrátí swapped=True, znamená to, že v LIVE řádku je
    request-away tým ve skutečnosti na HOME pozici. V tom případě musíme použít
    HOME-goals model (FTHG), aby mean odpovídal request-away.
    """
    ...
    def predict_from_match(
        ...
        *,
        ...
        # pokud je swapped, request-away je v řádku na HOME pozici -> použij HOME model
        if swapped:
