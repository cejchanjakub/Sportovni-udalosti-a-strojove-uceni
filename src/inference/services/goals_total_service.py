...
from src.inference.model_loader import load_goals_total_model
from src.line_generator import generate_ou_lines_around_mean, pick_main_line, to_dicts
@dataclass
class GoalsTotalResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet gólů (lambda)
    lines: Dict[str, Any]  # O/U linky (fair + volitelně s marginem)
    matched_row: Dict[str, Any]


class GoalsTotalService:
    """
    Service pro total_goals (Poisson GLM).
    ...
    # konfigurace dynamických linek pro goals_total
    ...
    def __init__(self) -> None:
        if self.__class__._model is None:
            model, scaler, feat_order = load_goals_total_model()
    ...
    def predict_from_match(
        ...
        *,
        ...
        # standardizace stejná jako při tréninku
        ...
        # ---- Dynamické O/U linky kolem mean (fair + volitelně margin) ----
