# src/inference/services/corners_home_service.py
...
from src.inference.model_loader import load_corners_home_model, load_corners_away_model
from src.line_generator import generate_ou_lines_around_mean, pick_main_line, to_dicts


@dataclass
class CornersHomeResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet rohů REQUEST-HOME
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class CornersHomeService:
    """
    Service pro rohy domácího týmu (request-home).

    Stejný pattern jako u CardsHomeService:
    - _pick_row může vrátit swapped=True (řádek je obráceně)
    - pokud swapped, request-home je v řádku jako AWAY => použij away model
    ...
    # konfigurace dynamických linek pro corners_home
    ...
    def predict_from_match(
        ...
        *,
        ...
        # pokud je swapped, request-home je v row jako AWAY -> použij away model
        if swapped:
