# src/inference/services/sot_total_service.py
...
from src.inference.model_loader import load_sot_total_model
from src.line_generator import generate_ou_lines_around_mean, pick_main_line, to_dicts


@dataclass
class SOTTotalResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet SOT celkem (lambda)
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class SOTTotalService:
    """
    Service pro total shots on target (SOT total) – Poisson GLM.

    - načte GLM artefakty (sot__total_sot)
    ...
    def __init__(self) -> None:
        if self.__class__._model is None:
            model, scaler, feat_order = load_sot_total_model()
