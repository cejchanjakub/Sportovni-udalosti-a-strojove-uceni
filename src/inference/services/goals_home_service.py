# src/inference/services/goals_home_service.py
...
from src.inference.model_loader import load_goals_home_model, load_goals_away_model
from src.line_generator import generate_ou_lines_around_mean, pick_main_line, to_dicts


@dataclass
class GoalsHomeResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet gólů REQUEST-HOME (lambda)
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class GoalsHomeService:
    """
    Service pro goals_home = góly domácích dle REQUESTU.

    Pozn.: pokud _pick_row vrátí swapped=True, znamená to, že v LIVE řádku je
    request-home tým ve skutečnosti na AWAY pozici. V tom případě musíme použít
    AWAY-goals model (FTAG), aby mean odpovídal request-home.
    """
    ...
    def predict_from_match(
        ...
        *,
        ...
        # pokud je swapped, request-home je v řádku na AWAY pozici -> použij AWAY model
        if swapped:
