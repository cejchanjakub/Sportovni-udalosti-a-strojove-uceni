...
from src.inference.model_loader import load_goals_home_model
...
class GoalsHomeService:
    ...
    def __init__(self) -> None:
        if self.__class__._model is None:
            model, feat_order, alpha = load_goals_home_model()
            self.__class__._model = model
            self.__class__._feat_order = feat_order
            self.__class__._alpha = alpha  # None pro Poisson, float pro NegBin

    def predict_from_match(self, utc_date: str, home_team: str, away_team: str, *, margin: float = 0.0) -> GoalsHomeResult:
