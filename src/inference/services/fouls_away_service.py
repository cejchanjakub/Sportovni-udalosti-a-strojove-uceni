# src/inference/services/fouls_away_service.py
...
@dataclass
class FoulsAwayResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet faulů REQUEST-AWAY
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class FoulsAwayService:
    """
    Service pro fauly hostujícího týmu (request-away).
    ...
    def predict_from_match(
        ...
        *,
        ...
        # swapped => request-away je v row jako HOME
        if swapped:
        ...
        if not math.isfinite(mean_pred) or mean_pred < 0:
            raise ValueError(f"FoulsAwayService: invalid mean_pred={mean_pred}")
