# src/inference/services/cards_away_service.py
...
from src.inference.model_loader import load_cards_home_model, load_cards_away_model
from src.line_generator import generate_ou_lines_around_mean, pick_main_line, to_dicts


@dataclass
class CardsAwayResult:
    mapped: Dict[str, str]
    swapped: bool
    mean: float  # očekávaný počet karet REQUEST-AWAY
    lines: Dict[str, Any]
    matched_row: Dict[str, Any]


class CardsAwayService:
    ...
    def predict_from_match(
        ...
        *,
        ...
        # pokud je swapped, request-away je v row jako HOME -> použij home model
        if swapped:
